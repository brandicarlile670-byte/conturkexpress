...
export default async function TrackingPage({searchParams}:{searchParams:Promise<{number?:string|string[]}>}){const p=await searchParams;const n=Array.isArray(p.number)?p.number[0]:p.number;if(!n?.trim())return <PublicTrackingLookup/>;const record=getTrackingByNo(n);if(!record)return <div className="ct-public-tracking"><div className="lookup-shell"><div className="header"><h1>Parcel Tracking</h1></div><p className="lookup-error">No tracking record was found for <strong>{n}</strong>.</p><a href="/tracking" className="tracking-back-link">Search again</a></div></div>;return <PublicTrackingDetails record={record}/>;}
